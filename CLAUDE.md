...
- Portfolio frequencies use official project counts and minted invocations, grouped by chain + contract + project. Held rarity means rank within the entire project, never wallet subsets. Default Portfolio missing traits and combinations on, but keep both independently editable. Apply changed options consistently to every whole-project score/rank, clear stale displayed values immediately, and round-trip options separately from Analyze settings. Total points are inventory sums, not cross-project rarity. Keep neutral colors and explicit 10,000 verified-piece cap/partial notices; no 25-project cutoff.
...
- Automatic full-project scores/ranks require all minted invocations, stable count/update signatures and project aggregates, complete supported metadata, resolved scores and matching held evidence including canonical pairs. Preserve official distributions; supplement missing whole types only. Both displayed score and rank use the same applied options. Incomplete baselines leave the requested score/rank unavailable. Show tie intervals and assumption counts. Limits: 20k/project, 50k/run, 1m pair observations/project; yield between batches. Filters/sorts never recompute ranks.
...
- Cache at most 50,000 verified population tokens in the current tab for option recalculation; retry uncached populations. C34’s supplied address is prefilled and editable; never auto-scan on load. Project disclosures preserve expansion and independent paging. Summary columns are identity, OS rarity and whole-project Held rarity only; no wallet-relative rank columns in CSV.
...
- Display-only `artBlocksProject` metadata is separate from Portfolio's `artBlocksVerified` gate. Validate exact chain/contract/token/project and official catalog before enrichment; never replace Analyze/Compare scoring fields.
- AB500 requires exact public heritage project tag `ab500`; unknown is not false. Never infer from project ID or `is_artblocks`. Preserve co-creator artist_name. Category comes from vertical, not curation_status. Curated Series comes from its public tag or series_id only for Curated.
- Show artist/classification under project titles everywhere. The OS rarity column contains only rarity; the OS collection label belongs in project identity. Mixed/listed headers describe verified projects in fetched results.
