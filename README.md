...
- Held rarity automatically ranks each owned artwork against its entire Art Blocks project, with both options checked by default and independently editable. Changing either recalculates current whole-project scores and ranks. Ties show rank/percentage ranges. Retry unavailable scores for a selected project or all unfinished projects (20,000 pieces/project; 50,000/run; one million pair observations/project).
- Project headings show artist, verified AB500 membership, category and numbered Curated Series where available. The OpenSea collection label sits below the title. Analyze and Compare labels describe the verified projects among fetched works, without changing scoring data.
- Portfolio bonus choices round-trip separately from Analyze choices in JSON settings and shareable links. Both default on; neither wallet nor API key is put into those settings.
...
**Score missing traits / unavailable data** is off by default in Analyze/Compare and enabled by default in Portfolio. When enabled:
...
Scores depend on each collection’s trait structure. Portfolio uses project minted supply and official feature frequencies, grouping by chain + contract + project ID, never shared core. Total held points are inventory sums, not cross-project rarity. Held rarity means only the full-project score rank; holdings-subset ranks are removed. OpenSea ranks remain separate and never become fallback points. Re-run Portfolio after changing tiers, weights or bonuses. Displayed points and ranks use the same selected-options baseline/config for every piece, complete stable data and tie ranges.
...
The regression suite uses synthetic fixtures and a non-browser DOM. Release 1.6.0 tests independently editable Portfolio options, cached recalculation, clearing stale scores, settings/link round-trips, exact-token label enrichment, AB500 tri-state membership, category/series traps, artist credits, placement in every view and CSV escaping. Live metadata checks cover all 500 official AB500 members and 67 numbered series, including Quine above project ID 500 and non-AB500 Studio/Flex. Existing scoring, startup and privacy regressions remain covered. No new browser UI or authenticated OpenSea test is claimed; no real key is present in the repository or fixtures.
