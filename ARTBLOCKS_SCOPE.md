...
5. Automatically fetch every project piece and score the whole population plus owned works with the selected missing-trait and combination options. Complete supported metadata establishes per-token presence and pairs. Unsupported structured fields or generating/unavailable aggregates cannot establish full scoring completeness.

Defaults remain **7/3/1 points below 2%/5%/20%**, with exclusive boundaries. Portfolio defaults both options on, independently of Analyze’s checkboxes, and lets you toggle either. A change recalculates all current holdings against their whole projects; verified populations are cached in this tab up to 50,000 pieces, and uncached projects are fetched again. Old score/rank displays are cleared immediately during recalculation. Preserve official feature-value distributions; supplement only whole missing types from the complete scan. Measured absence uses per-token presence, never aggregate value sums. Unavailable individual frequencies may receive assumed rare points, labelled N/A and excluded from measured coverage. The top three rarest eligible pairs use the combination multiplier. No OpenSea rank becomes custom points.
...
CSV includes project identity, independent ranking populations, tie endpoints, percentages, assumptions, dates, configuration fingerprint and both applied flags. Wallet-subset rank columns are removed. Summary rows follow selected-project/search filters without changing full-project rank populations.
...
## Project identity, artist, AB500 and series

Project identity areas show artist, AB500 membership and category across Analyze, Compare, Portfolio summaries, project disclosures, cards, tables and CSV. The OpenSea collection label appears below the project title rather than in its rarity cell. A header listing “projects in these results” describes fetched NFTs, not an unverified classification of an entire mixed OpenSea collection.

The official [GraphQL reference](https://docs.artblocks.io/developer/graphql/) provides project/artist/vertical relationships. Exact chain, contract, token and nested project identities plus live catalog membership are mandatory. Labels are attached as separate display metadata and never replace OpenSea traits, scores, supply, ownership, prices or collection identity.

- AB500 membership uses the exact public, heritage, project tag `ab500`. It is never inferred from project number or `is_artblocks`. A complete tag array without that entry means not AB500; missing or malformed membership metadata stays unverified.
- Category uses the official vertical, not legacy `curation_status`, which can incorrectly say factory for Studio/Flex. Categories include Curated, Factory, Playground, Presents, Explorations, Studio, Engine/Flex and branded Collaborations.
- Numbered Curated Series uses its public heritage tag, or a positive series ID only when the vertical is Curated. Factory/Playground series IDs are not Curated Series.
- Preserve `artist_name`, including co-creators. Artist profiles are only a fallback.
- Optional label lookup failures never discard existing scores. No metadata labels are added to the scoring trait set.

The [official AB500 collection](https://www.artblocks.io/discover/ab-500) covers the historical 500-project collection, not the first 500 numeric IDs on every core. Live review on 2026-09-04 verified all 500 members and 67 numbered series. Quine (project 506) is included; Factory/Playground projects with unrelated series IDs are not mislabelled.
